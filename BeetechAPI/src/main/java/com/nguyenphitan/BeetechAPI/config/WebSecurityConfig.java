...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/", "/swagger-ui.html", "/resources/**", "/vendor/**", "/css/**",
				"/js/**", "/fonts/**", "/images/**", "/static/**", "/assets/**").permitAll();
		http.authorizeRequests()
				.antMatchers("/public/**", "/list-cart", "/cart", "/clone/**", "/list-products/**", "/details/**")
				.permitAll();
		http.authorizeRequests().antMatchers("/admin/**", "/api/csv/**", "/add-list/**").hasAnyAuthority("ROLE_ADMIN");
